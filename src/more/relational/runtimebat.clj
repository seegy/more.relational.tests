...
(defn create-employee-database-onlyfk
  ...
  (let [raw-data (load-raw-data)
        ...
        xrel-emp (get_emps_by_manager xrel-dept-man (:xrel-emp raw-data) base-count)
        emp_nos (into #{} (map #(:emp_no %) xrel-emp))
        emp-relvar (batrel/batvar (batrel/convertToBats [:emp_no :birth_date :first_name :last_name :gender :hire_date] (take employee-count xrel-emp)) )
        dept-relvar (batrel/batvar (batrel/convertToBats [:dept_no :dept_name] (:xrel-department raw-data)) )
        xrel-sal (empno_filter emp_nos (:xrel-sal raw-data))
        xrel-titles (empno_filter emp_nos (:xrel-titles raw-data))
        xrel-dept-emp (empno_filter emp_nos (:xrel-dept_emp raw-data))
        sal-relvar (batrel/batvar (batrel/convertToBats [:emp_no :salary :from_date :to_date] xrel-sal) #{
                                                                                                           {:foreign-key {:key :emp_no,
                                                                                                                          :referenced-relvar emp-relvar,
                                                                                                                          :referenced-key :emp_no}}})

        title-relvar  (batrel/batvar (batrel/convertToBats [:emp_no :title :from_date] xrel-titles) #{
                                                                                                       {:foreign-key {:key :emp_no,
                                                                                                                      :referenced-relvar emp-relvar,
                                                                                                                      :referenced-key :emp_no}}})

        dept-man-relvar  (batrel/batvar (batrel/convertToBats [:dept_no :emp_no :from_date] xrel-dept-man) #{
                                                                                                              {:foreign-key {:key :emp_no,
                                                                                                                             :referenced-relvar emp-relvar,
                                                                                                                             :referenced-key :emp_no}}
                                                                                                              {:foreign-key {:key :dept_no,
        ...
        dept-emp-relvar  (batrel/batvar (batrel/convertToBats [:emp_no :dept_no :from_date] xrel-dept-emp) #{
                                                                                                              {:foreign-key {:key :emp_no,
                                                                                                                             :referenced-relvar emp-relvar,
                                                                                                                             :referenced-key :emp_no}}
                                                                                                              {:foreign-key {:key :dept_no,
                            ...
                            (apply mapv +
                                   (mapv (fn[x]
                                           (let[database (create-employee-database-onlykey employee-count)
                                             ...
                                             (map (fn[[k v]] (batrel/constraint-reset! v (filter #(= (ffirst %) :key) (get constr k)))) database)
                                             (employee-manipulation database)))
                                         (range counter)))))
                            ...
                            (apply mapv +
                                   (mapv (fn[x]
                                           (let[database (create-employee-database-onlyfk employee-count)
                                                constr (constraints database)]
                                             (map (fn[[k v]] (batrel/constraint-reset! v (filter #(= (ffirst %) :foreign-key) (get constr k)))) database)
                                             (employee-manipulation database)))
                                         (range counter)))))
                            ...
                            (apply mapv +
                                   (mapv (fn[x]
                                           (let[database (create-employee-database-nocons employee-count)
                                                constr (constraints database)]
                                             (map (fn[[k v]] (batrel/constraint-reset! v #{})) database)
                                             (employee-manipulation database)))
                                         (range counter)))))))
